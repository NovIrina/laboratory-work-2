...
def load_models(path_to_original_model: Path, path_to_compressed_model: Path, batch_size: int) -> tuple[HFLM, HFLM]:
...
def check_memory_footprint(original_model: HFLM, compressed_model: HFLM) -> None:
    ...
    compression_ratio = original_model_memory_footprint / compressed_model_memory_footprint
    print(f"Compression Ratio: {compression_ratio}")
    return compression_ratio


def check_quality(path_to_original_model: Path, path_to_compressed_model: Path, original_model: HFLM, compressed_model: HFLM, batch_size: int) -> None:
...
def print_report(compression_ratio: float, performance_drop: float, score: float) -> None:
    print(f"Compression Ratio: {compression_ratio}")
